...
class State(StateMain):  # StateMain을 상속받아 하나로 합침 (중복 방지)
    ...
    # -----------------------------
    # 이미지 업로드 → (Gemini) 필드 추출 → 자동 입력
    # -----------------------------
    @rx.event
    def set_image_target(self, value: str):
        self.image_target = value
    ...
    async def handle_image_upload(self, files: list[rx.UploadFile]):
        ...
        if not files:
            self.image_upload_error = "업로드할 이미지 파일을 선택해주세요."
            return
        ...
        try:
            ...
            prompt = (
                "다음 이미지는 SAP SE11 테이블 구조 캡처입니다.\n"
                "이미지에서 필드 목록을 추출해 JSON만 출력하세요. 다른 설명은 절대 포함하지 마세요.\n"
                "출력 형식:\n"
                "{\n"
                '  "fields": [\n'
            ...
            # Map to target table structure: Selection or ALV
            target = (self.image_target or "").strip()
            rows = []
            for item in fields:
                ...
                rows.append({"field_id": fid, "label": lbl, "is_key": is_key})

            if not rows:
                raise ValueError("필드명(field_id)을 추출하지 못했습니다.")

            if "조회조건" in target:
                n = self._apply_selection_rows(
                    [
                        {"section": "SE11", "label": r.get("label", ""), "field_id": r.get("field_id", ""), "input_type": "단일"}
                        for r in rows
                    ],
                    mode="replace",
                )
                self.image_upload_info = f"조회조건 테이블에 {n}건을 자동 반영했습니다. (필요 시 직접 수정 가능)"
            elif "영역 B" in target or target.endswith("B"):
                n = self._apply_alv_rows(
                    [{"label": r.get("label", ""), "field_id": r.get("field_id", ""), "is_key": r.get("is_key", False)} for r in rows],
                    area="B",
                    mode="replace",
                )
                self.image_upload_info = f"ALV(영역 B) 테이블에 {n}건을 자동 반영했습니다."
            else:
                n = self._apply_alv_rows(
                    [{"label": r.get("label", ""), "field_id": r.get("field_id", ""), "is_key": r.get("is_key", False)} for r in rows],
                    area="A",
                ...
                self.image_upload_info = f"ALV(영역 A) 테이블에 {n}건을 자동 반영했습니다."
        except Exception as e:
            self.image_upload_error = f"이미지 자동 입력 실패: {e}"
...
""".strip()
    ...
    def _tbd_signal(md: str) -> dict[str, bool]:
        ...
        """
        sections = StateMain._split_md_sections(md)
